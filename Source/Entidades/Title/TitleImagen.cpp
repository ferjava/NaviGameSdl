...
entt::entity Factory::Title::createTitleImagen(entt::registry &reg,
  ...
  SDL_FRect title_dest{0.0f, 0.0f, (float)title_sp->w, (float)title_sp->h};
  auto title_img = reg.create();
  reg.emplace<GC::Posicion>(title_img,
                            ctx.pantalla.w / 2 - (float)title_sp->w / 2,
                            ctx.pantalla.h / 3 - (float)title_sp->h / 2);
